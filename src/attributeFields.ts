...
export const attributeFields = (Model: any, options: any = {}) => {
  const cache = options.cache || {};
  const result = Object.keys(Model.rawAttributes).reduce((memo: any, key) => {
    if (options.exclude) {
    ...
    memo[key] = {
      type: toGraphQL(type, Model.sequelize.constructor),
    };
